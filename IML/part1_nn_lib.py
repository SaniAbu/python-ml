...
class SigmoidLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        return grad_z * (self.sigmoid(self._cache_current) * (1 - self.sigmoid(self._cache_current)))
...
class ReluLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        mask = (self._cache_current > 0).astype(float)
        return grad_z * mask
...
class LinearLayer(Layer):
    ...
    def forward(self, x):
        ...
        output_data = np.matmul(x, self._W) + self._b
        self._cache_current = x.copy()
    ...
    def backward(self, grad_z):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        grad_inputs = np.matmul(grad_z, np.transpose(self._W))
...
class MultiLayerNetwork:
    ...
    def __init__(self, input_dim, neurons, activations):
        ...
        """
        self.input_dim = input_dim # e.g. 4
        self.neurons = neurons # e.g. [16, 3]
        self.activations = activations # e.g. "relu", "identity", "sigmoid"
...
class Trainer:
    ...
    def shuffle(input_dataset, target_dataset):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        p = np.random.permutation(input_dataset.shape[0])
    ...
    def train(self, input_dataset, target_dataset, val_input=None, val_target=None):
        """
        ...
        for _ in range(self.nb_epoch):
            ...
            for i in range(0, len(input_dataset), self.batch_size):
                ...
                loss = self._loss_layer(output_batch, target_batch)
                grad_loss = self._loss_layer.backward()
                
                self.network.backward(grad_loss)
                self.network.update_params(self.learning_rate)

            # evaluate loss at the end of the epoch (full-dataset evaluation)
            try:
                train_loss = self.eval_loss(input_dataset, target_dataset)
            except Exception:
                train_loss = None
            ...
            if val_input is not None and val_target is not None:
                try:
                    val_loss = self.eval_loss(val_input, val_target)
                except Exception:
                    val_loss = None
    ...
    def eval_loss(self, input_dataset, target_dataset):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        # only for final eval so do full network pass
...
class Preprocessor:
    ...
    def apply(self, data):
        ...
        normalized_data = (data - self.min_values) / range_values
    ...
    def revert(self, data):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        denormalized_data = data * (self.max_values - self.min_values) + self.min_values
